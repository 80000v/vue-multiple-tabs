<template>
	<div class="tab-view-wrap">
		<el-tag class="tag-view" v-for="item in tagList" :key="item.id" :class="{ active: item.name === $route.name }"  @click.native="jump(item)"  @close="close(item.name)" closable="true">{{ item.meta.title }}</el-tag>
	</div>
</template>
